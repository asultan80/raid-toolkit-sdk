...
* Added StaticTypesExtension to account export — hero-types.static.json, skill-types.static.json, and artifact-types.static.json are now included when exporting/importing an account via the tray Dump button; previously these static files were only accessible via WebSocket API
* Fixed skill-types.static.json never populated — `EffectType.get_IsContainer()` is a computed IL2CPP getter that calls `CallMethod` via gRPC (unavailable under binary fallback); defaulted to `false` in `EffectType.ToModel()`; all 5420 skill types now serialized successfully
* Fixed hero-types.static.json never populated — `AllSkillTypeIds` is a computed IL2CPP property whose getter calls `CallMethod` via gRPC (unavailable under binary fallback); set `SkillTypeIds = null` in `HeroType.ToModel()` so hero type data (name, faction, rarity, role, base stats) is captured without requiring gRPC; all 8177 champion types now serialized successfully
* Fixed gRPC DeadlineExceeded on C# array types (`T[]`) — `HeroForm[]` and `EffectKindId[]` carry no `[Generated]` attribute, don't implement `IRuntimeObject`, and aren't Nullable<T>, so they fell through to gRPC which always timed out; added `mt.IsArray` and `typeof(IRuntimeObject).IsAssignableFrom(mt)` to the skip list in `TryGetOrLoadTypeInfoCore`; C# arrays and IRuntimeObject wrapper types are read directly via native ReadMemory without field offset lookup
* Fixed artifacts empty due to Nullable&lt;T&gt; type resolution triggering gRPC — Nullable&lt;int&gt;/Nullable&lt;long&gt; are .NET framework types not in the binary type map; added to gRPC skip list in TryGetOrLoadTypeInfoCore so they return null without a 10-second timeout; NullableFactory rewritten to read hasValue/value directly from known IL2CPP memory offsets instead of using Il2CppTypeInfoLookup which requires type info to be present; ArtifactBonus.ToModel() and Artifact.ToModel() hardened against null _value, _primaryBonus, and _secondaryBonuses fields; ArtifactStorageResolver classPtr resolved via x64 RIP-relative instruction scan (TryGetClassPtrFromMethodCode) since MetadataUsages is null in IL2CPP v31
